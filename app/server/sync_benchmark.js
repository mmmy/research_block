...
async function main() {
  ...
  const dates_rows = await bocai_client.query(`select * from ssq order by time ${whereClause}`)
  ...
  if (dates_rows && dates_rows.length > 0) {
    ...
    dates_rows.forEach(async (row, i) => {
      const { date, id, time } = row
      // ssq 是北京时间晚上21：15开奖，utc为13:15, 找到之后的第一个出块
      const blocks = await getBtcBlocksByTime(`${date}T13:15:00Z`, 1)
      if (blocks.length > 0) {
